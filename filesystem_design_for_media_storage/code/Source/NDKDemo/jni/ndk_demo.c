...
JNIEXPORT jstring JNICALL Java_com_marakana_NativeLib_details
  (JNIEnv * env, jobject obj,jstring path) {
		
	const char *appPath = (*env)->GetStringUTFChars(env, path, 0);
	sprintf(fullPath,"%s/storage/free.txt",appPath);
		
	int i=0, x=0;
	FILE *p;
	char c;
 
 	p=fopen(fullPath,"rb+");													//free.txt
    while((c=fgetc(p)) != EOF)
	{  
		if(c=='0')	x++;
	}
...
JNIEXPORT jstring JNICALL Java_com_marakana_NativeLib_hello
  (JNIEnv * env, jobject obj,jstring path) {
		...
		char delalloc[100];
		sprintf(delalloc,"%s/allocation.txt",fullPath);
		FILE *p = fopen(delalloc,"w");        
		fclose(p);
		initialise_free(); 
